			...
			if (foundError == false) {
				...
				if (rec.QRS_Type == "PATIENTSET") {
					...
					for (var i=0;i<t2.length; i++) {
						...
						console.log(t2);
						console.log(curItem);
						console.log(i2b2.h.isBadObjPath('i2b2.IDRTAdditionalData.cfg.config.plugin'));
						if ((t2[i].checked == true) && (rec.size > 0) && (curItem == " idrt additional data") )
//								&& !(i2b2.h.isBadObjPath('i2b2.IDRTAdditionalData.cfg.config.plugin'))
//								) 
								
								{

									i2b2.hive.MasterView.setViewMode('Analysis');
									i2b2.PLUGINMGR.ctrlr.main.selectPlugin("IDRTAdditionalData");
									...
									var sdxData = {};
									sdxData[0] = i2b2.sdx.Master.EncapsulateData('PRS', rec);							
//									i2b2.IDRTAdditionalData.prsDropped(sdxData);
									i2b2.IDRTAdditionalData.Dropped(sdxData);
									i2b2.IDRTAdditionalData.setShowMetadataDialog(false);
